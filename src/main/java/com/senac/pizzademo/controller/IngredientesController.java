...
import com.senac.pizzademo.dto.IngredientesDTO;
...
public class IngredientesController {
    ...
    public List<IngredientesDTO> getAllIngredientes() {
        ...
        return ingredientesRepository.findAll().stream()
            .map(ing -> modelMapper.map(ing, IngredientesDTO.class))
            .toList();
    }
    ...
    @PostMapping
    public IngredientesDTO createIngrediente(@Valid @RequestBody IngredientesDTO ingredientesDTO) {
        logger.info("Criando ingrediente: {}", ingredientesDTO.getIngrediente());
        Ingredientes entity = modelMapper.map(ingredientesDTO, Ingredientes.class);
        Ingredientes saved = ingredientesRepository.save(entity);
        return modelMapper.map(saved, IngredientesDTO.class);
    }
    ...
    @PostMapping("/batch")
    public List<IngredientesDTO> createMultiplosIngredientes(@RequestBody List<IngredientesDTO> ingredientesList) {
        logger.info("Criando múltiplos ingredientes");
        List<Ingredientes> entities = ingredientesList.stream()
            .map(dto -> modelMapper.map(dto, Ingredientes.class)).toList();
        return ingredientesRepository.saveAll(entities).stream()
            .map(ing -> modelMapper.map(ing, IngredientesDTO.class)).toList();
    }
    ...
    @PutMapping("/{id}")
    public IngredientesDTO updateIngrediente(@PathVariable Long id, @RequestBody IngredientesDTO ingredientesDTO) {
        logger.info("Atualizando ingrediente com ID {}: {}", id, ingredientesDTO.getIngrediente());
        return ingredientesRepository.findById(id)
            .map(existing -> {
                existing.setIngrediente(ingredientesDTO.getIngrediente());
                existing.setQuantidade(ingredientesDTO.getQuantidade());
                return modelMapper.map(ingredientesRepository.save(existing), IngredientesDTO.class);
            })
    ...
    @PatchMapping("/{id}")
    public IngredientesDTO updateIngredienteParcial(@PathVariable Long id, @RequestBody IngredientesDTO ingredientesDTO) {
        logger.info("Atualizando parcialmente ingrediente com ID {}", id);
        return ingredientesRepository.findById(id)
            .map(existing -> {
                if (ingredientesDTO.getIngrediente() != null) {
                    existing.setIngrediente(ingredientesDTO.getIngrediente());
                }
                if (ingredientesDTO.getQuantidade() != null) {
                    existing.setQuantidade(ingredientesDTO.getQuantidade());
                }
                return modelMapper.map(ingredientesRepository.save(existing), IngredientesDTO.class);
            })
